...
# remove population from reference
bipolar <- SetAllIdent(bipolar, id = 'celltype')
bipolar <- SubsetData(bipolar, ident.remove = 'BC6', subset.raw = TRUE)

ordering <- c("Amacrine cells", "BC1A", "BC1B", "BC2", "BC3A", "BC3B", "BC4", "BC5A", "BC5B", "BC5C", "BC5D", "BC7", "BC8/BC9",
              "Cone photoreceptors", "Müller glia", "RBC", "Rod  photoreceptors", "BC6")
...
evaluate_accuracy <- function(tab) {
  tab[tab$true_label == 'BC6',]$true_label <- 'unassigned'
  accuracy <- sum(tab$predicted.id == tab$true_label) / nrow(tab) * 100
  return(accuracy)
}
...
accuracy_full <- evaluate_accuracy(projection_full)
projection[projection$prediction.score < 0.5, 'predicted.id'] <- 'unassigned'
accuracy_threshold <- evaluate_accuracy(projection)

within(projection_full, true_label <- factor(true_label, levels = ordering)) %>% 
  ggplot(., aes(prediction.score, fill = true_label)) +
    geom_histogram(bins=25) +
    facet_wrap(~true_label, scales = 'free_y') +
    geom_vline(xintercept = 0.5) +
    theme(legend.position = 'none') +
  ggsave("figures/bipolar_projection_no_bc6_scores.pdf")
...
seurat_projection <- within(seurat_projection, true_label <- factor(true_label, levels=ordering))
seurat_projection_full <- within(seurat_projection_full, true_label <- factor(true_label, levels=ordering))
...
scmap_cluster_accuracy_threshold <- evaluate_accuracy(scmap.cluster.results)
...
scmap_cluster_threshold <- within(scmap_cluster_threshold, true_label <- factor(true_label, levels=ordering))
...
rownames(scmap.cluster.results) <- rownames(colData(bipolar6.sce))
scmap.cluster.results$true_label <- as.character(sampled_celltypes[rownames(scmap.cluster.results)])
scmap_cluster_accuracy_no_threshold <- evaluate_accuracy(scmap.cluster.results)
...
scmap_cluster_full <- within(scmap_cluster_full, true_label <- factor(true_label, levels=ordering))
...
scmap_cell_accuracy <- evaluate_accuracy(scmap.cell.results)
...
scmap_cell_threshold <- within(scmap_cell_threshold, true_label <- factor(true_label, levels=ordering))
...
rownames(scmap.cell.results) <- rownames(colData(bipolar6.sce))
scmap.cell.results$true_label <- as.character(sampled_celltypes[rownames(scmap.cell.results)])
scmap_cell_accuracy_no_threshold <- evaluate_accuracy(scmap.cell.results)
...
scmap_cell_no_threshold <- within(scmap_cell_no_threshold, true_label <- factor(true_label, levels=ordering))
...
plot_grid(seurat_threshold, scmap_cluster_threshold_plot, scmap_cell_threshold_plot, nrow=1) + ggsave("figures/bipolar_projection_thresholds_missing_bc6.pdf")
plot_grid(seurat_full, scmap_cluster_full_plot, scmap_cell_full_plot, nrow=1) + ggsave("figures/bipolar_projection_full_missing_bc6.pdf")
