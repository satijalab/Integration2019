...
for(i in 1:length(experiments)) {
  project.name <- unlist(x = strsplit(x = experiments[[i]], "/"))[[2]]
  dat <- read_data(base_path = paste0("raw_data/spatial/starmap/", experiments[[i]]),
                   project = project.name)
  saveRDS(object = dat, file = paste0("seurat_objects/", project.name, ".rds"))
}
